...
(defn sigv2 [{:keys [request-method uri headers body query-string server-name] :as req}
             {:keys [secret-key access-key region service token payload]}]
  (let [qp
...
(comment
  ...
  ;; AKIAI35BNDF74BDQZUMQ,SSZ5+KC7tKPy/0B6b8GM0EJ+N4UCxleJmQ8RLbOr
  ;; this is where I am testing my request
  ;; And it' not WORKING !!!
  (let [host "sdb.eu-west-1.amazonaws.com"
    ...
    (->> qp
         ...
         (clj-http.client/post (str "https://" host))))
